...
curl -X POST --form file=@/storage/external_SD/screenshots/root_before.txt --form press=upload http://192.168.111.142:8000 || true
...
curl -X POST --form file=@$FILE_PATH --form press=upload http://192.168.111.142:8000 || true
#rm $FILE_PATH
stat -c "%s" $FILE_PATH
...
find /storage/external_SD/screenshots/ -type f -print | while read -r file ; do 
  echo "uploading $file"
  curl -X POST --form file=@$file --form press=upload http://192.168.111.142:8000 &
  sleep 2
  rm $file 
done

tail -n 300 /data/crontab/root.log > /storage/external_SD/screenshots/root_after.txt || true
curl --form file=@/storage/external_SD/screenshots/root_after.txt --form press=upload http://192.168.111.142:8000 &
sleep 2
